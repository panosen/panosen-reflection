...
namespace Panosen.Reflection
{
    ...
    {
        public static MainModel Load(Assembly assembly, XmlDoc xmlDoc)
        {
            MainModel mainModel = new MainModel();

            mainModel.ClassNodeList = LoadClassList(assembly, xmlDoc);

            return mainModel;
        }

        public static List<ClassNode> LoadClassList(Assembly assembly, XmlDoc xmlDoc = null)
        {
            var types = assembly.GetTypes();

            List<ClassNode> items = ToClassNodeList(types, xmlDoc?.Members);

            return items;
        }

        private static List<ClassNode> ToClassNodeList(Type[] types, List<XmlMember> xmlMembers)
        {
            List<ClassNode> classNodeList = new List<ClassNode>();

            foreach (var type in types)
            {
                ClassNode classNode = ToClassNode(type, xmlMembers);
        ...
        private static ClassNode ToClassNode(Type type, List<XmlMember> xmlMembers)
        {
